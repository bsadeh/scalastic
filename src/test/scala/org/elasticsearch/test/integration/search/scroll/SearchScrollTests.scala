...
class SearchScrollTests extends IndexerBasedTest {
  ...
  test("simpleScrollQueryThenFetch") {
    ...
    hits.getTotalHits should be === (100)
    hits.hits.length should be === (35)
    for (hit <- hits) hit.sortValues()(0).asInstanceOf[Number].longValue() should be === (counter += 1)
  ...
  test("simpleScrollQueryThenFetchSmallSizeUnevenDistribution") {
    ...
    for (hit <- response.hits) {
      hit.sortValues()(0).asInstanceOf[Number].longValue() should be === (counter += 1)
    }
    for (i <- 0 until 32) {
      response = indexer.searchScroll(response.scrollId, scroll = Some("2m"))
      response.hits.getTotalHits should be === (100)
  ...
  test("scrollAndUpdateIndex") {
    ...
    indexer.waitForGreenStatus()
    for (i <- 0 until 500) {
      indexer.index(indexName, "tweet", i.toString, """{"user": "kimchy", "postDate": %s, "message": "test"}""".format(i))
    }
    indexer.refresh()
    ...
    indexer.count(query = termQuery("message", "update")).count should be === (0)
    var response = indexer.search_prepare().setQuery(queryString("user:kimchy"))
      .setSize(35)
      .setScroll(TimeValue.timeValueMinutes(2))
    ...
    do {
      for (searchHit <- response.hits.hits) {
        val map = searchHit.sourceAsMap()
        map.put("message", "update")
        indexer.index(indexName, "tweet", searchHit.id(), searchHit.sourceAsString)
      }
      response = indexer.searchScroll(response.scrollId, scroll = Some("2m"))
