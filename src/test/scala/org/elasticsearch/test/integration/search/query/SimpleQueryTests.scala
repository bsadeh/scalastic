...
class SimpleQueryTests extends IndexerBasedTest {
  ...
  test("passQueryAsStringTest") {
    ...
    indexer.refresh()
    indexer.search(query = termQuery("field1", "value1_1")).hits.totalHits should be === (1)
  }
  ...
  private def typeFilterTests(index: String) {
    ...
    indexer.count(Nil, Seq("type1")).count() should be === (2)
    indexer.count(Nil, Seq("type2")).count() should be === (3)
    indexer.count(Nil, Seq("type1", "type2")).count() should be === (5)
  }
  ...
  private def idsFilterTests(index: String) {
    ...
    (response.hits.hits() map (_.id)).toSet should be === Set("1", "3")
    ...
    response.hits.totalHits should be === (0)
  }
  ...
  test("filterExistsMissingTests") {
    ...
    (response.hits.hits() map (_.id)).toSet should be === Set("1", "2")
    ...
    (response.hits.hits() map (_.id)).toSet should be === Set("4")

    response = indexer.search(query = filteredQuery(matchAllQuery, missingFilter("field1")))
    (response.hits.hits() map (_.id)).toSet should be === Set("3", "4")
