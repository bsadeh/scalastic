...
class SimpleQueryTests extends IndexerBasedTest {
  ...
  private def typeFilterTests(index: String) {
    ...
    indexer.refresh()
    indexer.count(indices = Set("_all"), query = filteredQuery(matchAllQuery, typeFilter("type1"))).getCount should equal (2)
    indexer.count(indices = Set("_all"), query = filteredQuery(matchAllQuery, typeFilter("type2"))).getCount should equal (3)
