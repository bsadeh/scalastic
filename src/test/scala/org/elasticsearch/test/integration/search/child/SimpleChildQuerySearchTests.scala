...
@org.junit.runner.RunWith(classOf[org.scalatest.junit.JUnitRunner]) class SimpleChildQuerySearchTests extends IndexerBasedTest {
  ...
  test("simpleChildQuery") {
    ...
    response.getHits.totalHits should equal (1)
    response.getHits.getAt(0).id should equal ("c1")
    response.getHits.getAt(0).field("_parent").value().toString should equal ("p1")
    ...
    //    response.getHits.getAt(0).id(), anyOf(equalTo("c1") should equal ("c2"))
    response.getHits.getAt(0).field("_parent").value().toString should equal ("p1")
    //    response.getHits.getAt(1).id(), anyOf(equalTo("c1") should equal ("c2"))
    response.getHits.getAt(1).field("_parent").value().toString should equal ("p1")
    ...
    response.getFailedShards should equal (0)
    response.getHits.totalHits should equal (1)
    response.getHits.getAt(0).id should equal ("p1")
    ...
    response.getFailedShards should equal (0)
    response.getHits.totalHits should equal (1)
    response.getHits.getAt(0).id should equal ("p2")
    response = indexer.search(Seq(indexName), query = topChildrenQuery("child", termQuery("c_field", "red")))
    response.getShardFailures.length should equal (0)
    response.getFailedShards should equal (0)
    response.getHits.totalHits should equal (2)
    //    response.getHits.getAt(0).id(), anyOf(equalTo("p2") should equal ("p1"))
    //    response.getHits.getAt(1).id(), anyOf(equalTo("p2") should equal ("p1"))
  ...
  test("testScopedFacet") {
    ...
    indexer.index(indexName, "child", "c4", """{"c_field": "red"}""", parent = "p2")
    indexer.refresh()
    val response: SearchResponse = indexer.search_prepare(Seq(indexName)).setQuery(topChildrenQuery("child", boolQuery().should(termQuery("c_field","red")).should(termQuery("c_field", "yellow")))
      .scope("child1"))
    ...
    //    response.getHits.getAt(0).id(), anyOf(equalTo("p2") should equal ("p1"))
    //    response.getHits.getAt(1).id(), anyOf(equalTo("p2") should equal ("p1"))
    response.facets().facets().size should equal (1)
    val termsFacet: org.elasticsearch.search.facet.terms.TermsFacet = response.facets().facet("facet1")
    termsFacet.entries().size should equal (2)
    termsFacet.entries().get(0).term() should equal ("red")
    termsFacet.entries().get(0).count() should equal (2)
    termsFacet.entries().get(1).term() should equal ("yellow")
    termsFacet.entries().get(1).count() should equal (1)
  } */
  ...
  test("testDeletedParent") {
    ...
    response.getHits.totalHits should equal (1)
    response.getHits.getAt(0).id should equal ("p1")
    response.getHits.getAt(0).sourceAsString() should include("\"p_value1\"")
    response = indexer.search(Seq(indexName), query = constantScoreQuery(hasChildFilter("child", termQuery("c_field", "yellow"))))
    response.getShardFailures.length should equal (0)
    response.getFailedShards should equal (0)
    response.getHits.totalHits should equal (1)
    ...
    response.getHits.totalHits should equal (1)
    response.getHits.getAt(0).id should equal ("p1")
    response.getHits.getAt(0).sourceAsString() should include("\"p_value1_updated\"")
    response = indexer.search(Seq(indexName), query = constantScoreQuery(hasChildFilter("child", termQuery("c_field", "yellow"))))
    response.getShardFailures.length should equal (0)
    response.getFailedShards should equal (0)
    response.getHits.totalHits should equal (1)
