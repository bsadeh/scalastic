...
@org.junit.runner.RunWith(classOf[org.scalatest.junit.JUnitRunner]) class SimpleChildQuerySearchTests extends IndexerBasedTest {
  ...
  test("simpleChildQuery") {
    ...
    response.getHits.totalHits should be === (1)
    response.getHits.getAt(0).id should be === ("c1")
    response.getHits.getAt(0).field("_parent").value().toString should be === ("p1")
    ...
    //    response.getHits.getAt(0).id(), anyOf(equalTo("c1") should be === ("c2"))
    response.getHits.getAt(0).field("_parent").value().toString should be === ("p1")
    //    response.getHits.getAt(1).id(), anyOf(equalTo("c1") should be === ("c2"))
    response.getHits.getAt(1).field("_parent").value().toString should be === ("p1")
    ...
    response.getFailedShards should be === (0)
    response.getHits.totalHits should be === (1)
    response.getHits.getAt(0).id should be === ("p1")
    ...
    response.getFailedShards should be === (0)
    response.getHits.totalHits should be === (1)
    response.getHits.getAt(0).id should be === ("p2")
    response = indexer.search(Seq(indexName), query = topChildrenQuery("child", termQuery("c_field", "red")))
    response.getShardFailures.length should be === (0)
    response.getFailedShards should be === (0)
    response.getHits.totalHits should be === (2)
    //    response.getHits.getAt(0).id(), anyOf(equalTo("p2") should be === ("p1"))
    //    response.getHits.getAt(1).id(), anyOf(equalTo("p2") should be === ("p1"))
