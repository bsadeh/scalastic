...
class SimpleSearchTests extends IndexerBasedTest {
  ...
  test("simpleIpTests") {
    ...
    indexer.refresh()
    indexer.search(query = boolQuery().must(rangeQuery("from") lt "192.168.0.7").must(rangeQuery("to") gt "192.168.0.7")).getHits.totalHits should equal (1)
  }
  ...
  test("simpleIdTests") {
    ...
    search(queryString("_id:XXX1")).getHits.totalHits should equal (1)
    search(prefixQuery("_id", "XXX")).getHits.totalHits should equal (1)
    search(queryString("_id:XXX*").lowercaseExpandedTerms(false)).getHits.totalHits should equal (1)
  }
  ...
  test("simpleDateRangeWithUpperInclusiveDisabledTests") {
    ...
    indexer.refresh()
    indexer.search(Seq(specialIndex), query = rangeQuery("field").gte("2010-01-05").lte("2010-01-06")).getHits.totalHits should equal (1)
    indexer.search(Seq(specialIndex), query = rangeQuery("field").gte("2010-01-05") lt "2010-01-06").getHits.totalHits should equal (1)
  }
  ...
  test("simpleDateMathTests") {
    ...
    indexer.index(indexName, "type1", "2", """{"field": "2010-01-06T02:00"}""")
    indexer.refresh()
    search(rangeQuery("field").gte("2010-01-05").lte("2010-01-06")).getHits.totalHits should equal (2)
    search(rangeQuery("field").gte("2010-01-05") lt "2010-01-06").getHits.totalHits should equal (1)
  }
  ...
  test("dateMath") {
    ...
    indexer.index(indexName, "type1", "2", """{"field":"2010-01-06T02:00"}""")
    indexer.refresh()
    indexer.search(query = rangeQuery("field").gte("2010-01-03||+2d").lte("2010-01-04||+2d")).getHits.totalHits should equal (2)
    indexer.search(query = queryString("field:[2010-01-03||+2d TO 2010-01-04||+2d]")).getHits.totalHits should equal (2)
  }
  ...
  test("text search") {
    ...
    search("field:bla").getHits.totalHits should equal (0)
    search("field:try").getHits.totalHits should equal (1)
    search("trying").getHits.totalHits should equal (2)
    search("trying to find an exact match").getHits.totalHits should equal (2)
    search("\"trying to find an exact match\"").getHits.totalHits should equal (1)

    search(matchPhraseQuery("field", "trying to find an exact match")).getHits.totalHits should equal (1)
    search(matchPhraseQuery("field", "\"trying to find an exact match\"")).getHits.totalHits should equal (1)
  }
  ...
  test("multiSearch") {
    ...
    responses(0).getResponse.getHits.totalHits should equal (1)
    responses(0).getResponse.getHits.getAt(0).id should equal ("1")
    responses(1).getResponse.getHits.totalHits should equal (1)
    responses(1).getResponse.getHits.getAt(0).id should equal ("2")
    responses(2).getResponse.getHits.totalHits should equal (2)
  }
}
