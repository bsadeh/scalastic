...
class IndexAliasesTests extends MultiNodesBasedTests {
  ...
  test("testAliases") {
    ...
    indexResponse.getIndex should be === (indexName)
    ...
    indexResponse.getIndex should be === ("test_x")
  }
  ...
  test("testSearchingFilteringAliasesTwoIndices") {
    ...
    indexer("server1").count(Seq("foos")).getCount should be === (2L)
    ...
    indexer("server1").count(Seq("bars")).getCount should be === (1L)
    ...
    indexer("server1").count(Seq("foos", "test1")).getCount should be === (5L)
    ...
    assertHits(response.getHits, "1", "2", "3", "4", "5")
    indexer("server1").count(Seq("foos", "aliasToTest1")).getCount should be === (5L)
    ...
    response.getHits.totalHits should be === (8L)
    indexer("server1").count(Seq("foos", "aliasToTests")).getCount should be === (8L)
    //logger.info("--> checking filtering alias for two indices and non-filtering alias for both indices")
    response = indexer("server1").search(Seq("foos", "aliasToTests"), query = termQuery("name", "something"))
    assertHits(response.getHits, "4", "8")
    indexer("server1").count(Seq("foos", "aliasToTests"), query = termQuery("name", "something")).getCount should be === (2L)
  }
  ...
  test("testDeletingByQueryFilteringAliases") {
    ...
    indexer("server1").count(Seq("test1")).getCount should be === (3L)
  ...
  test("testWaitForAliasCreationMultipleShards") {
    ...
    clusterHealth.getStatus should be === (ClusterHealthStatus.GREEN)
    for (i <- 0 until 10) {
      indexer("server1").alias(Seq(indexName), "alias%s".format(i)).isAcknowledged should be === (true)
  ...
  test("testWaitForAliasSimultaneousUpdate") {
    ...
    for (i <- 0 until aliasCount) {
      ...
      executor.submit(new Runnable() {
        override def run() {
          indexer("server1").alias(Seq(indexName), aliasName).isAcknowledged should be === (true)
  ...
  test("testWaitForAliasTimeout") {
    ...
    clusterHealth.isTimedOut should be === (false)
    clusterHealth.getStatus should be === (ClusterHealthStatus.GREEN)
    indexer("server2").alias(Seq(indexName), "alias1", timeout = Some("0ms")).isAcknowledged should be === (false)
    indexer("server1").alias(Seq(indexName), "alias2", timeout = Some("0ms")).isAcknowledged should be === (false)
    indexer("server2").alias(Seq(indexName), "alias3").isAcknowledged should be === (true)
  ...
  test("testSameAlias") {
    ...
    indexer("server2").alias(Seq(indexName), "alias1").isAcknowledged should be === (true)
    val timeout = TimeValue.timeValueSeconds(2)
    //logger.info("--> recreating alias1 ")
    val stopWatch = new StopWatch()
    stopWatch.start()
    indexer("server2").alias(Seq(indexName), "alias1", timeout = Some(timeout.toString)).isAcknowledged should be === (true)
    stopWatch.stop().lastTaskTime().millis() should be < (timeout.millis())
    //logger.info("--> modifying alias1 to have a filter")
    stopWatch.start()
    indexer("server2").alias(Seq(indexName), "alias1", filter = Some(termFilter("name", "foo")), timeout = Some(timeout.toString)).isAcknowledged should be === (true)
    stopWatch.stop().lastTaskTime().millis() should be < (timeout.millis())
    ...
    stopWatch.start()
    indexer("server2").alias(Seq(indexName), "alias1", filter = Some(termFilter("name", "bar")), timeout = Some(timeout.toString)).isAcknowledged should be === (true)
    stopWatch.stop().lastTaskTime().millis() should be < (timeout.millis())
    ...
    stopWatch.start()
    indexer("server2").unalias(Seq(indexName), "alias1", timeout = Some(timeout.toString)).isAcknowledged should be === (true)
    stopWatch.stop().lastTaskTime().millis() should be < (timeout.millis())
