...
class IndexLifecycleActionTests extends MultiNodesBasedTests {
  ...
  test("testIndexLifecycleActionsWith11Shards1Backup") {
    ...
    routingNodeEntry1.numberOfShardsWithState(STARTED) should equal (11)
    ...
    routingNodeEntry2.numberOfShardsWithState(STARTED) should equal (11)
    ...
    routingNodeEntry1 = clusterState1.readOnlyRoutingNodes().nodesToShards()
      .get(clusterState1.nodes().localNodeId())
    //    routingNodeEntry1.numberOfShardsWithState(STARTED), anyOf(equalTo(7) should equal (8))
    clusterState2 = clusterService2.state()
    routingNodeEntry2 = clusterState2.readOnlyRoutingNodes().nodesToShards()
      .get(clusterState2.nodes().localNodeId())
    //    routingNodeEntry2.numberOfShardsWithState(STARTED), anyOf(equalTo(7) should equal (8))
    ...
    routingNodeEntry3.numberOfShardsWithState(STARTED) should equal (7)
    routingNodeEntry1.numberOfShardsWithState(STARTED) + routingNodeEntry2.numberOfShardsWithState(STARTED) + routingNodeEntry3.numberOfShardsWithState(STARTED) should equal (22)
    closeNode("server1")
    ...
    routingNodeEntry3 = clusterState3.readOnlyRoutingNodes().nodesToShards()
      .get(clusterState3.nodes().localNodeId())
    routingNodeEntry3.numberOfShardsWithState(STARTED) should equal (11)
    routingNodeEntry2.numberOfShardsWithState(STARTED) + routingNodeEntry3.numberOfShardsWithState(STARTED) should equal (22)
  ...
  test("testIndexLifecycleActionsWith11Shards0Backup") {
    ...
    var routingNodeEntry3 = clusterState3.readOnlyRoutingNodes().nodesToShards()
      .get(clusterState3.nodes().localNodeId())
    routingNodeEntry3.numberOfShardsWithState(STARTED) should equal (3)
    routingNodeEntry1.numberOfShardsWithState(STARTED) + routingNodeEntry2.numberOfShardsWithState(STARTED) + routingNodeEntry3.numberOfShardsWithState(STARTED) should equal (11)
    closeNode("server1")
    clusterHealth = indexer("server3").health_prepare().setWaitForGreenStatus().setWaitForNodes("2").setWaitForRelocatingShards(0).execute.actionGet
    clusterHealth.isTimedOut should equal (false)
    clusterHealth.getStatus should equal (ClusterHealthStatus.GREEN)
    ...
    routingNodeEntry2.numberOfShardsWithState(STARTED) + routingNodeEntry3.numberOfShardsWithState(STARTED) should equal (11)
    val deleteIndexResponse = indexer("server2").deleteIndex(Seq(indexName))
    deleteIndexResponse.isAcknowledged should equal (true)
    pending //fixme: failing test
    Thread.sleep(500)
