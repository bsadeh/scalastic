...
@org.junit.runner.RunWith(classOf[org.scalatest.junit.JUnitRunner])class SimpleDataNodesTests extends MultiNodesBasedTests {
  ...
  test("testDataNodes") {
    ...
    indexer("nonData1").waitForNodes(howMany = "2").timedOut() should be === (false)
    try {
    ...
    indexer("nonData1").waitForNodes(howMany = "3").timedOut() should be === (false)
    val indexResponse = indexer("nonData2").index(indexName, "type1", "1", source("1", "test"))
    indexResponse.id should be === ("1")
    indexResponse.`type`() should be === ("type1")
  }
